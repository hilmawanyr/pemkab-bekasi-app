...
module.exports = {
    ...
    async createNew(req, res) {
        try {
            ...
            const user = new User(username, pass, role, email, 1, req.session._authData.username);
            console.log(user)
    ...
    async update(req, res) {
        try {
            let { updateusername, email, role } = req.body;
            let updUser = await updateUser(updateusername, email, role, update);
            return res.redirect('/user');
        } catch (err) {
            return res.status(500).json({
                status: 88,

...
module.exports = {
    ...
    async showDetail(issue) {
        try {
            let detail = sequelize.query(`SELECT i.*, u.username, p.name AS project_name, p.code AS project_code FROM issues i JOIN projects p ON i.project_id = p.id JOIN users u ON i.created_by = u.id WHERE i.id = :id`, {
                replacements: {
                    id: issue
                },
                plain: true,
